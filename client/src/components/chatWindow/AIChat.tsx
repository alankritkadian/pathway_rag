...
const AiChat: React.FC<AiChatProps> = ({
  ...
  const saveChatToFirestore = async () => {
    ...
    try {
      // Sanitize chat history to replace undefined `thought` fields with null
      const sanitizedChatHistory = chatHistory.map((message) => ({
        ...message,
        thought: message.thought === undefined ? null : message.thought, // Replace undefined with null
      }));
  ...
  return (
    <div className="flex flex-col w-[700px] border-transparent rounded-lg mr-auto ml-5 h-screen bg-white/20 backdrop-blur-sm">
